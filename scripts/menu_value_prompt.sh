...
menu_value_prompt() {
    ...
    if [[ ${CI:-} == true ]] && [[ ${TRAVIS:-} == true ]]; then
        ...
        SELECTEDVALUE=$(whiptail --fb --title "Value Prompt" --menu "What would you like set for ${SET_VAR}?${VALUEDESCRIPTION:-}" --fb 0 0 0 "${VALUEOPTIONS[@]}" 3>&1 1>&2 2>&3)
        reset || true
    fi
    ...
    case "${SELECTEDVALUE}" in
        ...
        "Use System")
            ...
            ;;
        "Enter New")
            INPUT=$(whiptail --fb --title "Enter New" --inputbox "What would you like set for ${SET_VAR}?" 0 0 "${CURRENT_VAL}" 3>&1 1>&2 2>&3)
            ;;
        *)
    ...
    case "${SET_VAR}" in
        *_ENABLED)
            if [[ ${INPUT} == "true" ]] || [[ ${INPUT} == "false" ]]; then
                ...
                whiptail --fb --title "Error" --msgbox "${INPUT} is not true or false. Please try again." 0 0
        ...
        *_NETWORK_MODE)
            if [[ -z ${INPUT} ]] || \
                ...
                whiptail --fb --title "Error" --msgbox "${INPUT} is not a valid network mode. Please try again." 0 0
                menu_value_prompt "${SET_VAR}"
            fi
            ;;
        *_PORT_*)
            if [[ ${INPUT} =~ ^[0-9]+$ ]] || [[ ${INPUT} -ge 0 ]] || [[ ${INPUT} -le 65535 ]]; then
                ...
                whiptail --fb --title "Error" --msgbox "${INPUT} is not a valid port. Please try again." 0 0
                menu_value_prompt "${SET_VAR}"
            fi
            ;;
        *DIR|*DIR_*)
            if [[ -d ${INPUT} ]]; then
                ...
                whiptail --fb --title "Error" --msgbox "${INPUT} is not a valid path. Please try again." 0 0
                menu_value_prompt "${SET_VAR}"
            fi
            ;;
        P[GU]ID)
            if [[ ${INPUT} =~ ^[0-9]+$ ]]; then
                ...
                whiptail --fb --title "Error" --msgbox "${INPUT} is not a valid ${SET_VAR}. Please try again." 0 0
                menu_value_prompt "${SET_VAR}"
            fi
            ;;
        *)
