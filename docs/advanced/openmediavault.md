...
OpenMediaVault (OMV) requires some special setup in order to install TrunkSTARTer (DS). DS staff have tested and confirmed the following installation method on OMV version 5.5.
...
We are now going to work on creating the directories for DS to use. You will need to create 4 shared folders and allow "Everyone read/write". The 4 directories will be called `trunkdata`, `storage`, `medialibrary` and `home`.
