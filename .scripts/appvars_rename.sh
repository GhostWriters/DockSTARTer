...
appvars_rename() {
    ...
    if [[ ${FROMAPP_ENABLED} == true ]] && [[ ${TOAPP_ENABLED} != true ]]; then
        ...
        sed -i "s/^\s*${FROMAPP^^}_/${TOAPP^^}_/i" "${COMPOSE_ENV}" || fatal "Failed to migrate vars from ${FROMAPP^^}_ to ${TOAPP^^}_\nFailing command: ${F[C]}sed -i \"s/^\\s*${FROMAPP^^}_/${TOAPP^^}_/i\" \"${COMPOSE_ENV}\""
