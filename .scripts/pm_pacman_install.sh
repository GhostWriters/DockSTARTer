...
pm_pacman_install() {
    ...
    eval "sudo pacman -Sy --noconfirm curl dialog git grep sed ${REDIRECT}" || fatal "Failed to install dependencies using pacman.\nFailing command: ${F[C]}sudo pacman -Sy --noconfirm curl dialog git grep sed"
}
