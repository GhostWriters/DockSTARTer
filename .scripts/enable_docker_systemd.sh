...
enable_docker_systemd() {
    ...
    if [[ -L "/sbin/init" ]]; then
        ...
        sudo systemctl enable docker > /dev/null 2>&1 || fatal "Failed to enable docker service.\nFailing command: ${F[C]}systemctl enable docker"
        info "Starting docker service."
        sudo systemctl start docker > /dev/null 2>&1 || fatal "Failed to start docker service.\nFailing command: ${F[C]}systemctl start docker"
    fi
}
