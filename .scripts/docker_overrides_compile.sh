...
docker_overrides_compile() {
    ...
    if [[ -f "${SCRIPTPATH}/compose/docker-compose.override.yml" ]]; then
        if ! grep -q '# GENERATED BY DOCKSTARTER' "${SCRIPTPATH}/compose/docker-compose.override.yml"; then
            info "Found user-created docker-compose.overrides.yml file. Moving to ${DOCKER_OVERRIDES_DIR}/original_overrides.yml"
    ...
    if [[ $(find ${DETECTED_HOMEDIR}/.config/docker_overrides/* -type f -prune | wc -l) -eq 0 ]]; then
