...
install_docker() {
    ...
    if [[ ${FORCE:-} == true ]] && [[ -n ${INSTALL:-} ]]; then
        ...
        INSTALLED_DOCKER=$( (docker --version 2> /dev/null || echo "0") | sed -E 's/docker version ([^,]*)(, build .*)?/\1/')
    fi
    if vergt "${MINIMUM_DOCKER}" "${INSTALLED_DOCKER}"; then
        ...
        if vergt "${AVAILABLE_DOCKER}" "${INSTALLED_DOCKER}"; then
            ...
            UPDATED_DOCKER=$( (docker --version 2> /dev/null || echo "0") | sed -E 's/docker version ([^,]*)(, build .*)?/\1/')
