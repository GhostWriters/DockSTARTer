...
override_backup() {
    if [[ -f "${SCRIPTPATH}/compose/docker-compose.override.yml" ]]; then
        ...
        ${FIND} "${DOCKER_VOLUME_CONFIG}/.compose.backups" -type f -name "docker-compose.override.yml.*" -mtime +3 -delete &> /dev/null || warn "Old '${C["File"]}docker-compose.override.yml${NC}' backups not removed."
    fi
}
